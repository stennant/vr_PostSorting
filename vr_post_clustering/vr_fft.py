...
def plot_power_spectrum_moves_light(prm, light_movement, nolight_movement, light_stationary, nolight_stationary, channel):
    ...
    if os.path.isfile(analysis_path + 'ps_stationary_movement.png') is False:
        ...
        power_spectrum_log(prm, light_movement, 30000, 'k', prm.get_filename(), x_lim=150, line_width=15, legend='stationary')
        ...
        ax.tick_params(axis='y', pad = 10, top='off', right = 'off', direction = 'out',width = 2, length = 8, labelsize =18)
        ax.tick_params(axis='x', pad = 10, top='off', right = 'off', direction = 'out',width = 2, length = 8, labelsize =18)
        power_spectrum_log(prm, light_stationary, 30000, 'k', prm.get_filename(), x_lim=150, line_width=15, legend='stationary')
...
def generate_theta(prm):
    ...
    plt.show()
    fig.savefig(prm.get_filepath() + 'Electrophysiology/track_location_analysis/fft/CH' + '_test_theta.png', dpi=200)
...
def plot_test_power_spectrum(prm, data):
    ...
    power_spectrum_log(prm, data, 30000, 'k', prm.get_filename(), title="$Power spectrum$", x_lim=130, line_width=15, legend='stationary')
