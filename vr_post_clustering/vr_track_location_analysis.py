...
def split_locations_speed(prm,middle_upper,upper, middle_lower, lower):
    ...
    #split data based on location along the track
    outbound_m_upper = middle_upper[np.where(np.logical_and(middle_upper[:,1] > 70, middle_upper[:,1] < 90))]
    outbound_upper = upper[np.where(np.logical_and(upper[:,1] > 70, upper[:,1] < 90))]
    outbound_m_lower = middle_lower[np.where(np.logical_and(middle_lower[:,1] > 70, middle_lower[:,1] < 90))]
    outbound_lower = lower[np.where(np.logical_and(lower[:,1] > 70, lower[:,1] < 90))]

    rz_m_upper = middle_upper[np.where(np.logical_and(middle_upper[:,1] > 90, middle_upper[:,1] < 110))]
    rz_upper = upper[np.where(np.logical_and(upper[:,1] > 90, upper[:,1] < 110))]
    rz_m_lower = middle_lower[np.where(np.logical_and(middle_lower[:,1] > 90, middle_lower[:,1] < 110))]
    rz_lower = lower[np.where(np.logical_and(lower[:,1] > 90, lower[:,1] < 110))]

    hb_m_upper = middle_upper[np.where(np.logical_and(middle_upper[:,1] > 110, middle_upper[:,1] < 130))]
    hb_upper = upper[np.where(np.logical_and(upper[:,1] > 110, upper[:,1] < 130))]
    hb_m_lower = middle_lower[np.where(np.logical_and(middle_lower[:,1] > 110, middle_lower[:,1] < 130))]
    hb_lower = lower[np.where(np.logical_and(lower[:,1] > 110, lower[:,1] < 130))]
...
def hit_miss_power_spectra_locations(prm,before_stop, after_stop, channel):
...
def power_spectra_speed(prm,data, channel):
    ...
    vr_track_location_plots.plot_power_spectrum_speed_gamma(prm, middle_upper[:,5],upper[:,5], middle_lower[:,5], lower[:,5], channel)
    ...
    vr_track_location_plots.plot_power_spectrum_speed(prm, m_upper,u, m_lower, l, channel)
