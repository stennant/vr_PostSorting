...
def get_pooled_data(number_of_days):
    ...
    stops_on_trials = []
    trial_num = 0
    path  = '/Users/sarahtennant/Work/Analysis/Opto/Spikes/Test_for_Klara/recordings/2017-12-05_14-32-19/Behaviour/Data/'

    for day in range(number_of_days):

        if os.path.isfile(path + "location" + str(day + 1) + ".npy") is False:
            print('its stopped here')
...
def get_spikes_on_trials_find_stops(location, number_of_trials, all_stops, track_beginnings):
    ...
    for trial in range(number_of_trials - 1):
        ...
        stops_on_trial_indices = (np.where((beginning <= all_stops) & (all_stops <= end)))
        stops_on_trial = np.take(all_stops, stops_on_trial_indices)
        stops_on_trial = stops_on_trial.astype(int)
        if len(stops_on_trial) > 0:
            stops = np.take(location, stops_on_trial)
